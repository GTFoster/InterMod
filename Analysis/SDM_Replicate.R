...
for(i in 1:length(files)){
  nm <- files[i]
  tifname <- gsub(".csv", ".tif", nm) %>% paste("suitabilityOutputs/", ., sep="")
  if (file.exists(tifname)) {
    message("Skipping existing file: ", i)
    next
  }
  
  Occ <- read.csv(file=paste("data/GBIF/occs/", nm, sep=""))
  if(nrow(Occ)<10){ #skip if no observations
